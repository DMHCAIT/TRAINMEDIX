...
export const HospitalExplorer: React.FC = () => {
  ...
  const { hospitals, selectedCity, setSelectedCity, setSelectedHospital, setActiveTab } = useApp();
  ...
  const filteredHospitals = hospitals.filter((hosp) => {
    const locationText = (hosp.cities && hosp.cities.length > 0 ? hosp.cities.join(', ') : (hosp.city || hosp.address || 'Unknown')).toLowerCase();
    const addressText = (hosp.address || hosp.city || 'Unknown').toLowerCase();
    ...
    const matchesCity =
      selectedCity === 'All' ||
      selectedCity === 'Other' ||
      (hosp.city ? hosp.city.toLowerCase().includes(selectedCity.toLowerCase()) : false) ||
      locationText.includes(selectedCity.toLowerCase()) ||
      addressText.includes(selectedCity.toLowerCase());
